...
С помощью этого дашборда менеджеры по сопровождению и менеджеры отдела сбыта (при оценке внедрения апробационных установок)
могут получить ответы на следующие вопросы:
...
Из ответов на перечисленные выше вопросы менеджеры могут сложить достаточно полную картину полезности внедренного К/ТЭ 
у того или иного клиента.

Неоспоримым преимуществом данного дашборда является то, что он предоставляет информацию ПО ВСЕМ подконтрольным установкам СРАЗУ.
...
- [billingDocs](066-for-managers.md#billig-docs);
...
Здесь следует помнить, что использование временных фильтров, равно как и использование фильтров в самом верху страницы 
...
Контроль на 20ое число каждого месяца сделан специально, чтобы у ответственного менеджера было достаточно времени для 
...
Данную таблицу имеет смысл использовать совместно с дашбордом [Поисковые запросы.](072-search-query.md)
...
Данная таблица отображает данные об установках и файлах, на которых обновление OperUP произошло более чем 3 дня назад. 
...
Второе применение данной таблицы - контроль за регулярностью получения OperUp на той и или иной установке, если на ней в 
...
Справа от графика присутствует легенда, где цветовым меткам с графика проведены соотнесения с конкретной подконтрольной 
установке.
...
Если требуется фильтры, в том числе временные, применять только к данному графику, то следует раскрыть только его - сделать 
это можно, кликнув по его названию и выбрав выпавшем меню пункт View.
...
Такое может быть видно в том случае, когда временной период указан больше чем 24ч (т.к. за каждым зашедшим в ПК на 24ч 
фиксируется его рабочее место).
...
Данные графики и таблицы помогут выяснить кто именно (чьи учетки) и в каком количестве нуждаются в своих собственных 
рабочих местах и, таким образом, целенаправленно идти уже к конкретным заинтересованным пользователям.
...
не все лицензии заняты (особенно если такая картина регулярная), то это повод к выявлению дополнительных пользователей/отделов 
клиента, которые еще не в курсе, что у них внедрена ИСС Кодекс/Техэксперт.
...
когда рабочих мест станет не хватать на всех, что, в свою очередь, приведет к допродажной работе и, в конечном счете, к 
увеличению КМ.
...
В первом случае графически оценивается динамика нагрузки на ПК, уровень участия всех или части учеток на короткой дистанции 
(1 месяц).
Во втором случае - оцениваются более длительные периоды, чтобы получить больше информации о прогнозировании и планировании 
работы с тем или иным клиентом.
...
а кого-то - замотивировать на бОльшую активность/вовлеченность.
...
График подвержен изменениям временного периода - можно установить любой временной отрезок (из предустановленных или задать 
свой с точностью до суток), чтобы получить данные по графику за конкретный период во времени из прошлого.

На самом графике так же можно курсором выделить заинтересовавший период, чтобы отобразить собранные значения только за 
выбранный период. 
...
Такая аналитическая информация может быть использована для поиска ответа на вопрос о востребованности установки у того 
или иной клиента.
...
График отображает количественные значения выгруженных из системы документов посредством отправки на печать тех или иных 
документов напрямую с оболочки ПК с конкретизацией до пользователя на той или иной подконтрольной СМАРТу установке.
...
и текущие (столбец current) количественные значения выгруженных документов с дифференциацией по конкретным учеткам на 
подконтрольных установках.
...
Этот график схож с графиком [Print statistic](066-for-managers.md#print-statistic) с той лишь разницей, что здесь ведется 
контроль за количеством выгруженных из системы документов через СОХРАНЕНИЕ документа в рамках часа.
...
использования всех лицензий в течение 1 дня", описанной [выше](066-for-managers.md#пользователи-и-количество-раз-которые-не-смогли-подключиться-из-за-использования-всех-лицензий-в-течение-1-дня), для определения конкретных учеток, которые не смоли подключиться.
...
к установленным БД за определенный срок на подконтрольных установках по всем зарегистрированным в нем учеткам (включая 
новоприбывших за указанный период).
...
_(коллега, думаю, ты и тут тоже понимаешь, почему так может быть)_

---

**ВНИМАНИЕ!** Общее количество пользователей вычисляется по статистике работы в течение месяца. 
Если брать промежуток графика за границами одного месяца, то могут быть неверные данные.
...
График является более информативной версией графика [Среднее число запросов kassist за месяц](062-kodeks-analytics.md#среднее-число-запросов-kassist-за-месяц).
...
Для более подробного изучения вопроса по использованию сервиса kAssist пользователями ПК К/ТЭ рекомендуется обратиться к
[одноименному дашборду.](078-kassist.md)
...
Вновь там появиться она может, если снова перестанет создавать запросы в БД И при этом 60 дней назад она также не 
создавала запросы в БД.
...
Карточки пользователя один из важных источников информации о самом пользователе.
...
[Дальше](067-upload-sysinfo-into-ASRP.md)
